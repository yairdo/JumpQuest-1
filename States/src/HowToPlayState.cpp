...
/*
	c-tor
*/
HowToPlayState::HowToPlayState(StateManager& manager, sf::RenderWindow& window,
	...
	m_howTo(Resources::getResourceRef().getTexture(howTo))
{
	...
	m_rect.setSize({ float(m_window.getSize().x-m_window.getSize().x* TENTH_PREC),
		m_window.getSize().y - getTitlePosY()*1.2f - (butHeight)*1.5f });
	...
	m_howTo.setPosition({ m_window.getSize().x / 20.f,getTitlePosY()});

	m_rect.setFillColor(BLACK_TRANSP_CLR);
	m_rect.setPosition({ m_window.getSize().x / 20.f,getTitlePosY() });
